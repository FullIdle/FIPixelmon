...
public class GuiListQuest extends Gui {
   ...
   public void drawText() {
      ...
      for(int i = this.startLine; i < this.startLine + this.displayLines; ++i) {
         if (i < this.lines.size() && i >= 0) {
            QuestProgressClient qpc = (QuestProgressClient)this.lines.get(i);
            if (qpc != null && qpc.getName() != null) {
               ...
               if (this.selectedQuest != null && this.selectedQuest == i && this.highlightSelectedLine) {
                  ...
                  GuiHelper.drawSquashedString(this.fRend, qpc.getName(), false, wf, (float)(this.xPos + 4 + 12), (float)(this.yPos + 2 + this.lineHeight * (i - this.startLine) + this.lineHeight / 4), this.selectedTextColor, true);
               } else {
                  GuiHelper.drawSquashedString(this.fRend, qpc.getName(), false, wf, (float)(this.xPos + 4 + 12), (float)(this.yPos + 2 + this.lineHeight * (i - this.startLine) + this.lineHeight / 4), this.selectedTextColor, false);
               }
   ...
   public QuestProgressClient mouseClicked(int mouseX, int mouseY, int mouseButton, int offsetY) {
      if (mouseX > this.xPos && mouseX < this.xPos + this.width && mouseY > this.yPos && mouseY < this.yPos + this.height) {
         ...
         if (index < this.lines.size() && index >= 0) {
            return (QuestProgressClient)this.lines.get(index);
         }
      }
