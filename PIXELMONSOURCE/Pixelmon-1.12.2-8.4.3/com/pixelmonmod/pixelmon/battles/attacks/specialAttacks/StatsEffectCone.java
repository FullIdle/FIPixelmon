...
public class StatsEffectCone extends StatsEffect {
   public StatsEffectCone() {
   }

   public StatsEffectCone(StatsType type, int value, boolean isUser) {
      super(type, value, isUser);
   }

   public AttackResult applyStatEffect(PixelmonWrapper user, PixelmonWrapper target, AttackBase a) {
      List affectedList = user.bc.getAdjacentPokemonAndSelf(this.isUser ? user : target);
      ...
      while(true) {
         while(true) {
            ...
            do {
               ...
               } while(affected.isSameTeam(user));
   ...
   private ArrayList getAffectedOpponents(PixelmonWrapper pw, MoveChoice choice) {
      return pw.bc.getAdjacentPokemonAndSelf(this.isUser ? pw : (choice.targets.size() > 0 ? (PixelmonWrapper)choice.targets.get(0) : null));
   }
}
