...
public class ContainerDeck extends Container {
   public final InventoryDeck inventory;
   private static final int INV_DECK_SIZE = 62;
   private static final int INV_TOTAL_SIZE = 88;
   private static final int HOTBAR_END_INDEX = 97;

   public ContainerDeck(EntityPlayer player, InventoryPlayer invPlayer, InventoryDeck inventory) {
      ...
      this.func_75146_a(new SlotDeckCardBack(this.inventory, 60, -23, 26));
      this.func_75146_a(new SlotDeckCoin(this.inventory, 61, -23, 44));

      for(i = 0; i < 3; ++i) {
         for(int j = 0; j < 9; ++j) {
            this.func_75146_a(new Slot(invPlayer, j + i * 9 + 9, 8 + j * 18, 102 + i * 18));
         }
      }

      for(i = 0; i < 9; ++i) {
         this.func_75146_a(new Slot(invPlayer, i, 8 + i * 18, 160));
      }
   ...
   public ItemStack func_184996_a(int slot, int dragType, ClickType clickTypeIn, EntityPlayer player) {
      if (slot >= 0 && this.func_75139_a(slot).func_75211_c() == player.func_184614_ca()) {
         return ItemStack.field_190927_a;
      } else {
         return this.inventory != null && this.inventory.isLocked && slot < 62 ? ItemStack.field_190927_a : super.func_184996_a(slot, dragType, clickTypeIn, player);
      }
   }
