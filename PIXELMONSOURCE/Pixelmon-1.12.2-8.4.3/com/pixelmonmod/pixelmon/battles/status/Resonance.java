...
public class Resonance extends Screen {
   ...
   public void applyEffect(PixelmonWrapper user, PixelmonWrapper target) {
      if (user.targetIndex == 0 || user.bc.simulateMode) {
         if (user.hasStatus(this.type)) {
            ...
            user.addTeamStatus(((Resonance)this.getNewInstance(turns)).withUser(user), user);
   ...
   public Resonance withUser(PixelmonWrapper user) {
      this.user = user;
      return this;
   }
   ...
   protected Screen getNewInstance(int effectTurns) {
      return new Resonance((PixelmonWrapper)null, effectTurns);
   }
}
