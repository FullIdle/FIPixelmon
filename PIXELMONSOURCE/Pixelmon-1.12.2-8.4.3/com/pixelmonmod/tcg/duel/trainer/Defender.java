...
public class Defender extends BaseTrainerEffect {
   ...
   public int modifyDamage(int damage, TrainerCardState trainer, GameServerState server) {
      return server.getPlayer(server.getNextTurn()).getActiveCard().getAttachments().contains(trainer) ? Math.max(0, damage - 20) : damage;
   }
