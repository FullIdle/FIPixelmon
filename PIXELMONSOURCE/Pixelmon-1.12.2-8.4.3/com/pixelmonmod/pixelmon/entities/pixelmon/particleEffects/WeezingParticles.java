...
public class WeezingParticles extends ParticleEffects {
   public void onUpdate(Entity2Client pixelmon) {
      ...
      if (!pixelmon.getAnimationVariables().hasInt(72)) {
         pixelmon.getAnimationVariables().setInt(72, RandomHelper.rand.nextInt(21) * -1);
      }
      ...
      if (count > 0 && count < 3) {
         ClientProxy.spawnParticle(EnumPixelmonParticles.koffing, pixelmon.field_70170_p, pixelmon.field_70165_t + (double)var6, pixelmon.field_70163_u + 2.5, pixelmon.field_70161_v + (double)var7, pixelmon.getPokemonData().isShiny());
      } else if (count > 3) {
         pixelmon.getAnimationVariables().setInt(72, RandomHelper.rand.nextInt(21) * -1);
      }
